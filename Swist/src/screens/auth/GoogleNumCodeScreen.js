...
const GoogleNumCodeScreen = () => {
  ...
  const { setAuthToken, setIsAuth, authToken, appToken, authKey, i18n } =
    useAuth();
  ...
  const sentCode = () => {
    validationGoogleCode(countryCode + phone, value, appToken, authKey)
      .then((res) => {
      ...
      .catch((err) => {
        console.error("validationGoogleCode error :", err);
      });
  };
    ...
    if (isAddedToken) {
      if (authToken) {
        ...
        getProfileInfo(authToken).then((res) => {
          ...
          if (!res.name) {
            dispatch(setName(res.name.split(" ")[1]));
            dispatch(setSurname(res.name.split(" ")[0]));
